-- A role or crew set by hand (profile correction, Section Head decision) is never replaced by a workbook import.
-- 1. employee_directory_v exposes where the current role came from (role_source, role_note) so the planner can see it.
-- 2. commit_import_batch step 3 no longer deletes or closes a manual current role; the import row is left unapplied.
--    (The planner already turns such a difference into a review note, so nothing reaches step 3 normally;
--    this is the database backstop.)
...
begin
  ...
  for r in select * from public.import_rows where batch_id = p_batch_id and entity_kind = 'leave_record' and outcome = 'changed' and payload ? 'rescheduled_from_id' order by seq loop
    ...
    if v_orig.id is null or (not v_orig.in_current_plan and not (v_orig.status = 'rescheduled' and v_orig.source_batch_id = p_batch_id)) then
  ...
  for r in select * from public.import_rows where batch_id = p_batch_id and entity_kind = 'leave_record' and outcome = 'changed' and payload ? 'leave_record_id' and payload->>'change_kind' = 'not_taken' order by seq loop
    select * into v_orig from public.leave_records where id = (r.payload->>'leave_record_id')::uuid;
    if v_orig.id is null or not v_orig.in_current_plan or v_orig.source_kind = 'manual' then
