-- Import: the monthly sheets are the real leave (Section Head rule, 23 Sep 2026).
-- commit_import_batch:
--   5c  a replacement record takes its source kind from the plan row (monthly_grid for marked days) and one original
--       block may be split into several taken pieces within one batch;
--   5e  new step 'not_taken': current leave not marked on the monthly sheets leaves the current plan as 'rescheduled'
--       (kept in history, never deleted); records entered by hand are never touched.
