...
export default function MovementsPage() {
  ...
  const view = useMemo(() => {
    ...
    const current = active.filter((m) => m.kind === 'temporary' && m.start_date <= today && (!m.end_date || m.end_date >= today));
    const upcoming = active.filter((m) => m.kind === 'temporary' && m.start_date > today).sort((a, b) => a.start_date.localeCompare(b.start_date));
    ...
    return { people, name, current, upcoming, permanent, past };
  }, [data, today]);

  const done = (m: string) => { setSheet(null); setFlash(m); load(); };
  const Row = ({ m, actions }: { m: CrewMovement; actions?: boolean }) => (
    <li className="flex items-center gap-3 py-2.5">
      <span className="flex shrink-0 items-center gap-1">{m.from_crew && <CrewBadge crew={m.from_crew} size="sm" />}<ArrowRight className="h-3.5 w-3.5 text-slate-400" /><CrewBadge crew={m.to_crew} size="sm" /></span>
      <span className="min-w-0 flex-1">
        ...
        <span className="block truncate text-xs text-slate-500">{m.kind === 'permanent' ? 'Permanent move' : 'Temporary cover'} · {range(m)}{m.status === 'cancelled' ? ' · cancelled' : ''}</span>
  ...
  return (
    <div>
      <PageHeader title="Shift Movements" subtitle="Temporary covers with another crew and permanent crew moves. Manpower counts each person in their crew on each date."
