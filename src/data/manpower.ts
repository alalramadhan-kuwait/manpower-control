...
import type { MpAbsence, MpAssignment, MpPerson, Role } from '@/core/manpower';
...
/** Active Section-1 people, every current-plan absence and every active Controller assignment overlapping [from, to]. */
export async function fetchManpowerInputs(from: string, to: string): Promise<{ people: MpPerson[]; absences: MpAbsence[]; assignments: MpAssignment[] }> {
  const [dir, lv, ca] = await Promise.all([
  ...
  return { people: (dir.data as EmployeeDirectoryRow[]).map(toMpPerson), absences: (lv.data as unknown as LeaveRow[]).map(toMpAbsence), assignments: (ca.data as ControllerAssignment[]).map(toMpAssignment) };
}
